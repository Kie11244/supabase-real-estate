...
const AddPropertyPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!project_slug || !title_th || !title_en || price === '' || bedrooms === '' || bathrooms === '' || size_sqm === '') {
    ...
    try {
      const generatedSlug = slugify(title_en);
      const { error: insertError } = await supabase.from('properties').insert([
        {
          title_th,
          title_en,
          slug_en: generatedSlug,
          project_slug,
          type,
          price: Number(price),
    ...
    <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl space-y-8">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/95 p-8 shadow-2xl shadow-slate-200/60">
          ...
          <div className="sm:col-span-2">
            <label htmlFor="title_en" className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">
              ชื่อประกาศ (ภาษาอังกฤษ)
            </label>
            <input
              type="text"
              id="title_en"
